...
class AIX(hissar: kSar, header: String) : OSParser(hissar, header) {
    var under_average = false

    companion object {
        private val log = LoggerFactory.getLogger(AIX::class.java)
    }
    ...
    override fun parse_header(s: String) {
        ...
        Hostname = columns[1]
        Hostname = columns[1]
        OSversion = columns[2] + "." + columns[3]
        MacAddress = columns[4]
        setDate(columns[5])
    }
    ...
    override fun parse(line: String, columns: Array<String>): Int {
        ...
        try {
            ...
            if ("DEVICE" != currentStat || "CPUS" == currentStat) {
