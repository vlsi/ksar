...
public class DateFormatHelperTest {
  // Generator, not a test: prints the date patterns that the JDK locales produce, to seed
  // AllParser.DATE_FORMATS. A normal test run skips it; enable it with
  // -Dksar.generateDateFormats=true to refresh the pattern list. The output depends on the
  // JDK's locale data.
  @Test
  @EnabledIfSystemProperty(named = "ksar.generateDateFormats", matches = "true")
